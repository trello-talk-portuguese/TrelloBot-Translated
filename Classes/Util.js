...
module.exports = (client) => {
  let Util = {
    ...
    sendError(message, e) {
      ...
      client.log("Erro no comando:\n", e);
      message.channel.send(`Erro! Reporte no servidor de suporte listado em \`${client.config.prefix}help\`!\n\`\`\`js\n${Util.filter(e.stack)}\`\`\``);
    },
    sendWebError(message, e) {
      switch (e.errorCode) {
        case "err":
          console.log("Web Error:\n", e);
          message.channel.send(`Ouve um erro ao processar esse comando! Reporte no servidor de suporte listado em \`${client.config.prefix}help\`!\n\`${Util.filter(e.err.text)}\``);
          break;
        case "statusfail":
          console.log("Web Status Fail:\n", e);
          message.channel.send(`Eu não posso fazer isso! \`${e.response.status}: ${e.errorText}\``);
          break;
        default:
          if (e.stack) {
            message.channel.send(`Erro! Reporte no servidor de suporte listado em \`${client.config.prefix}help\`!\n\`\`\`js\n${Util.filter(e.stack)}\`\`\``);
