...
module.exports = class Card extends Command {
  ...
  async exec(message, args, { user }) {
    ...
    let embed = {
      ...
      url: body.shortUrl,
      description: "**Quadro**: [" + body.board.name + "](" + body.board.shortUrl + ")",
      author: {
        name: body.name,
    ...
    if (body.attachments.length > 0) {
      a = "\n**Anexos**: " + layout.attachments(body.attachments).join(", ");
      embed.fields.push({
        name: "Anexos",
    ...
    if (body.members.length > 0) {
      ...
      if (memberString.length > 50) {
        memberString = body.members.length + " Membros";
      }
      m = "\nMembros: " + memberString;
      embed.fields.push({
        name: "Membros",
        value: memberString,
        inline: true
    ...
    if (body.membersVoted.length > 0) {
      ...
      if (memberVoteString.length > 50) {
        memberVoteString = body.membersVoted.length + " Membros";
      }
      v = "\nMembros que votaram: " + memberVoteString;
      embed.fields.push({
        name: "Membros que votaram",
  ...
  get helpMeta() {
    return {
      ...
      description: "Mostra as informações de um cartão.",
      usage: ["<cardID>"]
    };
  }
};
